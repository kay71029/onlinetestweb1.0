<?php
  ...
  require('defense.php');
   ...
   /*--------------------------------------------------------
    ...
    ---------------------------------------------------------*/
  $sql_se_student_se_str = "SELECT * FROM `student` WHERE `user_id`='".$_SESSION['user_id']."'";
  $result_se_student_se_str = mysql_query($sql_se_student_se_str);
...
<div class="panel panel-primary">
  ...
  <div class="panel-body">
    ...
    <form method = "post" action ="userModifypost.php">    
        <?php while ($row =mysql_fetch_array($result_se_student_se_str)){?>
        <div class="input-group input-group-lg"> 
           <span class="input-group-addon" id="sizing-addon1">學號</span></span>
           <input type="text" class="form-control" VALUE="<?PHP echo $row[0];?>" placeholder="UserID" aria-describedby="sizing-addon1" name="fid" disabled required >
        </div>
          <br>
        <div class="input-group input-group-lg"> 
            <span class="input-group-addon" id="sizing-addon1">姓名</span></span>
            <input type="text" class="form-control" VALUE="<?PHP echo $row[1];?>" placeholder="UserName" aria-describedby="sizing-addon2" name="fname" disabled required  >
        </div>
          <br>
        <div class="input-group input-group-lg"> 
            <span class="input-group-addon" id="sizing-addon1">密   碼</span></span>
            <input type="password" class="form-control" VALUE="<?PHP echo $row[2];?>" placeholder="Password" aria-describedby="sizing-addon3" name="fpwd" required="">
        </div>
          <br>
        <div class="input-group input-group-lg"> 
        ...
        <div class="input-group input-group-lg"> 
            ...
            <input type="email" class="form-control" VALUE="<?PHP echo $row[3];?>" placeholder="Email" aria-describedby="sizing-addon5" name="femail" required="">
        </div>
          <br>
          <div class="input-group input-group-lg"> 
            <span class="input-group-addon" id="sizing-addon1">電話</span></span>
            <input type="text" class="form-control" VALUE="<?PHP echo $row[4];?>" placeholder="0910-123456" aria-describedby="sizing-addon5" name="ftel" required="">
        </div>
        <?php }?>
